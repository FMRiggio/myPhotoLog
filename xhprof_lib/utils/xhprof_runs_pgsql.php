...
class XHProfRuns_Default implements iXHProfRuns {
  ...
  {
	global $_xhprof;
	$connection = ("host=" . $_xhprof['dbhost'] . " dbname=" . $_xhprof['dbname'] . " user=" . $_xhprof['dbuser'] . " password=" . $_xhprof['dbpass']);
	$linkid = pg_connect($connection);
...
*/
  ...
  */
  public function getRuns($stats)
  {
      ...
      foreach($stats AS $column => $value)
      {
          ...
          $value = pg_escape_string($this->linkID, $value);
          $query .= " $column = '$value' ";
      }
      ...
      $resultSet = pg_query($this->linkID, $query);
      return $resultSet;
  }
  ...
  public function getHardHit($criteria)
  {
    ...
    unset($criteria['type']);
    $criteria['where'] = "DATE_SUB(CURDATE(), INTERVAL {$criteria['days']} DAY) <= time_stamp";
    unset($criteria['days']);
  ...
  {
	$sql['column'] = pg_escape_string ($this->linkID, $data['column']);
	$query = "SELECT DISTINCT(" . $sql['column'] . ") FROM details";
	$rs = pg_query($this->linkID, $query);
	return $rs;
  }
  ...
  */
  public function get_run($run_id, $type, &$run_desc) 
  {
    $run_id = pg_escape_string($this->linkID, $run_id);
  ...
  */
  ...
  {
      $data['select'] = 'id, UNIX_TIMESTAMP(timestamp) as time_stamp, pmu, wt, cpu';   
  ...
  */
  ...
  {
      $url = pg_escape_string($this->linkID, $url);
      $c_url = pg_escape_string($this->linkID, $c_url);
      //Runs same URL
      ...
      $row['95(wt)'] = $this->calculatePercentile(array('count' => $row['count'], 'column' => 'wt', 'type' => 'url', 'url' => $url));
      $row['95(cpu)'] = $this->calculatePercentile(array('count' => $row['count'], 'column' => 'cpu', 'type' => 'url', 'url' => $url));
      $row['95(pmu)'] = $this->calculatePercentile(array('count' => $row['count'], 'column' => 'pmu', 'type' => 'url', 'url' => $url));
      ...
      $row = pg_fetch_assoc($rs);
      $row['url'] = $c_url;
      $row['95(wt)'] = $this->calculatePercentile(array('count' => $row['count'], 'column' => 'wt', 'type' => 'c_url', 'url' => $c_url));
      $row['95(cpu)'] = $this->calculatePercentile(array('count' => $row['count'], 'column' => 'cpu', 'type' => 'c_url', 'url' => $c_url));
      $row['95(pmu)'] = $this->calculatePercentile(array('count' => $row['count'], 'column' => 'pmu', 'type' => 'c_url', 'url' => $c_url));
  ...
  */
    public function save_run($xhprof_data, $type, $run_id = null, $xhprof_details = null) 
    {
		...
		*/
        ...
        $sql['get'] = pg_escape_string($this->linkID, serialize($_GET));
        $sql['cookie'] = pg_escape_string($this->linkID, serialize($_COOKIE));
        
        //This code has not been tested
        if ($_xhprof['savepost'])
        {
        	$sql['post'] = pg_escape_string($this->linkID, serialize($_POST));    
        }else
        {
        	$sql['post'] = pg_escape_string($this->linkID, serialize(array("Skipped" => "Post data omitted by rule")));
        }
...
// full production code. 
        ...
        $sql['data'] = pg_escape_string($this->linkID, serialize($xhprof_data));
        
        $url   = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : $_SERVER['PHP_SELF'];
        $sname = isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : '';
	
        $sql['url'] = pg_escape_string($this->linkID, $url);
        $sql['c_url'] = pg_escape_string($this->linkID, _urlSimilartor($_SERVER['REQUEST_URI']));
        $sql['servername'] = pg_escape_string($this->linkID, $sname);
        $sql['type']  = (int) (isset($xhprof_details['type']) ? $xhprof_details['type'] : 0);
        $sql['time_stamp'] = pg_escape_string($this->linkID, $_SERVER['REQUEST_TIME']);
		    $sql['server_id'] = pg_escape_string($this->linkID, $_xhprof['servername']);

        $query = "INSERT INTO details (id, url, c_url, time_stamp, server_name, perfdata, type, cookie, post, get, pmu, wt, cpu, server_id) "
		       . "VALUES(   '" . $run_id . "'"
                   . ", '" . $sql['url']   . "'"
                   . ", '" . $sql['c_url'] . "'"
                   . ", '" . date('Y-m-d H:i:s', $sql['time_stamp']) . "'"
                   . ", '" . $sql['servername'] . "'"
                   . ", '" . $sql['data'] . "'"
                   . ",  " . $sql['type']
                   . ", '" . $sql['cookie'] . "'"
        ...
        if (pg_affected_rows($result) == 1) {          
            ...
            if ($_xhprof['display'] === true)
            {
                ...
                var_dump(pg_last_error($this->linkID));
            }
            return -1;
        }
  }
