	...
	**/
	class redux_auth extends redux_auth_db
	{
		...
		**/
		...
		/**
		 * Restituisce dati utente in base all'id
		 * 
		 */
		public function getUser($idUser = null) {
			if ($this->ci->session->userdata('id')) {
				return $this->_getUser($this->users_table, $this->ci->session->userdata('id'));
			}
			return false;
		}
	}
	...
	**/
	class redux_auth_db
	{
		...
		**/
		 ...
		 * 
		 */
		protected function _getUser ($users_tbl, $id_user)
		{
			$i = $this->ci->db->select()
			                  ->from($users_tbl)
			                  ->where($users_tbl.'.id', $id_user)
			                  ->limit(1)
			                  ->get();
			return $var = ($i->num_rows() > 0) ? $i->row() : false;
		}
